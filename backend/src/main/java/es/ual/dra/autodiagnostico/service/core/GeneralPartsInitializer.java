            ...
            if (root != null && root.isArray()) {
                for (JsonNode node : root) {
                    ...
                    if (node.has("priceRange")) {
                        ...
                        try {
                            pr = node.get("priceRange").asText();
                            String[] range = pr.split("-", 2);
                            minPrice = Double.parseDouble(range[0].trim());
                            maxPrice = Double.parseDouble(range[1].trim());
                        } catch (Exception e) {
                    ...
                    if (name != null) {
                        Product p = Product.builder()
                                ...
                                .lowRangePrice(minPrice)
                                .highRangePrice(maxPrice)
                                .image(image)
                                .build();
